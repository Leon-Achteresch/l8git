...
export function BranchSection({
  ...
  const isEmpty =
    grouping.flat.length === 0 && grouping.groups.length === 0;

  return (
    <section className="flex w-full min-w-0 max-w-full flex-col overflow-x-hidden">
      <header className="mb-1 grid w-full min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 px-2">
          ...
          {grouping.groups.length > 0 && (
            ...
            >
              {grouping.groups.map((g) => (
                ...
                >
                  <AccordionTrigger
                    className="group/trigger my-px flex w-full min-w-0 max-w-full items-center justify-start gap-1 rounded-md py-1 pl-2 pr-1.5 text-left text-[11px] font-medium tracking-wide text-muted-foreground transition-colors hover:bg-sidebar-accent/30 hover:text-foreground hover:no-underline data-[state=open]:text-foreground [&>svg]:shrink-0 [&>svg]:text-muted-foreground/80"
                  >
