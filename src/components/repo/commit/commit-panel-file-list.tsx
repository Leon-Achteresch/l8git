...
function buildListItems(
  ...
  if (stagedRows.length > 0) {
    items.push({ type: "header", label: labels.staged, count: stagedRows.length });
  ...
  if (unstagedRows.length > 0) {
    ...
    items.push({ type: "header", label: labels.unstaged, count: unstagedRows.length });
...
function VirtualFileListInner({
  ...
  const listItems = useMemo(
    ...
    [conflictRows, stagedRows, unstagedRows, t, i18n.language],
  );
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      <div className="flex items-center gap-3 border-b border-border/60 px-4 py-2.5">
        ...
        <span className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
          {t("commitPanel.fileListAllFiles")}
        </span>
        {conflictRows.length > 0 && (
          ...
          >
            ...
            {t("commitPanel.conflictBadge", { count: conflictRows.length })}
          </button>
        )}
