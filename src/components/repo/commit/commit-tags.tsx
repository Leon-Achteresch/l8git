  ...
  return (
    <>
      {tags.map((t) => (
        <span
          key={t}
          className="inline-flex min-w-0 max-w-full items-center gap-1 rounded-full border border-git-modified/30 bg-git-modified/15 px-1.5 py-px font-mono text-[0.625rem] font-medium text-git-modified"
          title={t}
        >
          <TagIcon className="h-3.5 w-3.5 shrink-0" />
          <span className="max-w-[7rem] truncate">{t}</span>
        </span>
      ))}
    </>
  );
