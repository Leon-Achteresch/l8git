...
import { DeltaBadge, LegendDot, PanelValue, RangePills } from "@/components/dashboard/panel-bits";
...
export function ActivityPanel({
  ...
  const { data, loading } = useActivityBuckets(path, days * 2, "day");
    ...
    <Card className={className}>
      <CardContent className="flex h-full flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <PanelValue
            value={totals.commits.toLocaleString(locale)}
