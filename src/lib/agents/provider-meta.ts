...
import { chatCapableAgentProviders } from "@/lib/agents/provider-registry";
import type { NativeAgentProvider } from "@/lib/agents/provider-store";

const NATIVE_PROVIDER_IDS: readonly NativeAgentProvider[] = ["codex", "claude", "opencode", "cursor"];

export const AGENT_PROVIDERS: ReadonlyArray<{
  value: NativeAgentProvider;
  label: string;
  description: string;
  Logo: ComponentType<{ className?: string }>;
}> = chatCapableAgentProviders()
  .filter((provider): provider is typeof provider & { id: NativeAgentProvider } =>
    NATIVE_PROVIDER_IDS.includes(provider.id as NativeAgentProvider),
  )
  .map((provider) => ({
    value: provider.id,
