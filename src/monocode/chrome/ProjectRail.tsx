    ...
    <div className="shrink-0 px-2">
      ...
      >
        <div className="flex items-center gap-2 px-3.5 py-1.5">
          <span
            aria-hidden
            className="size-1.5 shrink-0 rounded-full bg-mono-accent shadow-[0_0_8px_var(--color-mono-accent)] animate-pulse"
          />
...
function ProjectCard({
  ...
  return (
    ...
    >
      {showStart ? (
        <div className="pointer-events-none absolute inset-x-2 top-0 z-20 h-0.5 rounded-full bg-mono-accent" />
      ) : null}
      {showEnd ? (
        <div className="pointer-events-none absolute inset-x-2 bottom-0 z-20 h-0.5 rounded-full bg-mono-accent" />
      ) : null}
      <button
        type="button"
        title={cardTitle}
