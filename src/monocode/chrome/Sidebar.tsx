...
function SidebarComponent({
  ...
  const workspaceTabItems = visibleTabs.map((itemId, index) => {
    ...
    return (
      ...
      >
        {showStart ? (
          <div className="pointer-events-none absolute inset-y-0 left-0 z-20 w-0.5 bg-mono-accent" />
        ) : null}
        {showEnd ? (
          <div className="pointer-events-none absolute inset-y-0 right-0 z-20 w-0.5 bg-mono-accent" />
        ) : null}
        <button
          type="button"
          role="tab"
                  ...
                  {sessionListEntries.map((entry, index) => {
                    ...
                    if (entry.kind === "folder") {
                      ...
                      return (
                        ...
                        >
                          {showFolderDropStart ? (
                            <div className="pointer-events-none absolute inset-x-1 top-0 z-20 h-0.5 rounded-full bg-mono-accent" />
                          ) : null}
                          {showFolderDropEnd ? (
                            <div className="pointer-events-none absolute inset-x-1 bottom-0 z-20 h-0.5 rounded-full bg-mono-accent" />
                          ) : null}
                          <div
          ...
          >
            <span className="relative">
              ...
              {inboxUnseen ? (
                <span
                  aria-hidden
                  className="absolute -right-0.5 -top-0.5 size-1.5 rounded-full bg-mono-accent"
                />
              ) : null}
            </span>
          </IconButton>
...
function FolderRow({
  ...
  return (
    ...
    >
      {dropTarget ? (
        <div className="pointer-events-none absolute inset-0 rounded-md bg-mono-accent/20" />
      ) : null}
      <span
      ...
      <span className="relative flex shrink-0 items-center gap-1 text-[11px] tabular-nums text-content/45">
        {!expanded && needsApproval ? (
          ...
          <TerminalSpinner className="inline-block w-3 select-none text-center text-[11px] leading-none text-mono-accent" />
        ) : !expanded && done ? (
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="relative min-w-0 flex-1 rounded bg-content/10 px-2 py-0.5 text-[13px] font-semibold leading-snug text-content outline-none ring-1 ring-mono-accent/40"
      />
  ...
  return (
    ...
    >
      <input
        ...
        onBlur={() => finish(true)}
        onKeyDown={onKeyDown}
        className="w-full rounded bg-content/10 px-2 py-1 text-[13px] font-semibold leading-snug text-content outline-none ring-1 ring-mono-accent/40"
      />
    </div>
  );
}
