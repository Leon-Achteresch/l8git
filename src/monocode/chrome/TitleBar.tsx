  ...
  return (
    <span className="flex shrink-0 items-center">
      {shown.map((harness, i) => (
        ...
        >
          {busy.has(harness) ? (
            <TerminalSpinner className="inline-block w-3.5 select-none text-center text-[11px] leading-none text-mono-accent" />
          ) : (
...
function TitleTabItem({
  ...
  return (
    ...
    >
      {showStart ? (
        <div className="pointer-events-none absolute inset-y-1.5 left-0 z-20 w-0.5 rounded-full bg-mono-accent" />
      ) : null}
      {showEnd ? (
        <div className="pointer-events-none absolute inset-y-1.5 right-0 z-20 w-0.5 rounded-full bg-mono-accent" />
      ) : null}
      <button
        type="button"
        title={tooltip}
    ...
    <button
      ...
      className={`grid size-6.5 place-items-center rounded-md ${
        disabled
          ? "text-content/25"
          : accent
            ? "text-mono-accent hover:bg-content/10"
            : active
