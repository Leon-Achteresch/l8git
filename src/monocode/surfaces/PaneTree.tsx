...
function PaneDropHint({ edge }: { edge: PaneEdge }) {
  const wash =
    edge === "left"
      ? "absolute inset-y-0 left-0 w-1/2 bg-mono-accent/15"
      : edge === "right"
        ? "absolute inset-y-0 right-0 w-1/2 bg-mono-accent/15"
        : edge === "top"
          ? "absolute inset-x-0 top-0 h-1/2 bg-mono-accent/15"
          : "absolute inset-x-0 bottom-0 h-1/2 bg-mono-accent/15";
  const line =
    edge === "left"
      ? "absolute inset-y-0 left-0 w-0.5 bg-mono-accent"
      : edge === "right"
        ? "absolute inset-y-0 right-0 w-0.5 bg-mono-accent"
        : edge === "top"
          ? "absolute inset-x-0 top-0 h-0.5 bg-mono-accent"
          : "absolute inset-x-0 bottom-0 h-0.5 bg-mono-accent";
  return (
