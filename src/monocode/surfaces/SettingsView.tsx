...
import { formatUsd } from "../lib/tokenCost";
import { entryCost, sumCost, useUsageLedger } from "../lib/usageLedger";

const CapabilitySyncStudio = lazy(() =>
  import("@/components/agents/capabilities/capability-sync-studio").then((m) => ({
    default: m.CapabilitySyncStudio,
  })),
);
const CapabilityMarketplace = lazy(() =>
  import("@/components/agents/capabilities/capability-marketplace").then((m) => ({
    default: m.CapabilityMarketplace,
  })),
);
    ...
    >
      ...
      >
        <div className="mx-auto w-full max-w-5xl px-8 py-8">
          ...
          {section === "sync" ? (
            <CapabilityPage cwd={cwd} kind="sync" />
          ) : null}
          {section === "marketplace" ? (
            <CapabilityPage cwd={cwd} kind="marketplace" />
          ) : null}
          {section === "archive" ? (
            <ArchivePage
              cwd={cwd}
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <SecondaryButton onClick={() => void onClick()} disabled={busy}>
          ...
          <ArrowDownCircle className="size-3.5 text-mono-accent" aria-hidden />
        ) : (
  ...
  return (
    <button
      ...
      className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${
        on ? "bg-mono-accent" : "bg-content/20"
      }`}
    >
      <span
...
function UsagePage() {
  ...
  const entries = useMemo(
    () => Object.values(ledger).sort((a, b) => b.updatedAt - a.updatedAt),
    [ledger],
  );
  const byHarness = useMemo(() => {
    const map = new Map<string, number>();
    for (const entry of entries) {
      map.set(entry.harness, (map.get(entry.harness) ?? 0) + entryCost(entry));
    }
    return [...map.entries()].sort((a, b) => b[1] - a[1]);
  }, [entries]);
  const stat = (label: string, value: string) => (
    <div className="rounded-lg border border-content/10 px-4 py-3">
      <div className="text-[11px] uppercase tracking-wide text-content/45">{label}</div>
      <div className="mt-1 text-[20px] tabular-nums text-content">{value}</div>
    </div>
  );
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-3 gap-3">
        {stat("Today", formatUsd(sumCost(entries, 1)))}
        {stat("Last 7 days", formatUsd(sumCost(entries, 7)))}
        {stat("Last 30 days", formatUsd(sumCost(entries, 30)))}
      </div>
      {byHarness.length ? (
        <div className="flex flex-wrap gap-2 text-[12px] text-content/60">
          {byHarness.map(([harness, usd]) => (
            <span key={harness} className="rounded-md bg-content/5 px-2 py-1">
              {HARNESS_LABEL[harness as HarnessId] ?? harness} · {formatUsd(usd)}
            </span>
          ))}
        </div>
      ) : (
        <p className="text-[13px] text-content/50">
          No usage recorded yet. Claude Code and Codex report token counts after each turn.
        </p>
      )}
      {entries.length ? (
        <table className="w-full text-[12px]">
          <thead className="text-left text-content/45">
            <tr>
              ...
              <th className="py-1 text-right font-normal">Cached</th>
              <th className="py-1 text-right font-normal">Output</th>
              <th className="py-1 text-right font-normal">API cost</th>
            </tr>
          </thead>
          <tbody>
            {entries.slice(0, 100).map((entry) => (
              <tr key={entry.sessionId} className="border-t border-content/5 text-content/80">
                <td className="max-w-[260px] truncate py-1.5 pr-3">
                  {entry.title || entry.sessionId.slice(0, 8)}
                </td>
                <td className="py-1.5 pr-3 text-content/55">{entry.model}</td>
                <td className="py-1.5 text-right tabular-nums">{formatTokens(entry.input)}</td>
                <td className="py-1.5 text-right tabular-nums">{formatTokens(entry.cacheRead)}</td>
                <td className="py-1.5 text-right tabular-nums">{formatTokens(entry.output)}</td>
                <td className="py-1.5 text-right tabular-nums">{formatUsd(entryCost(entry))}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function CapabilityPage({ cwd, kind }: { cwd: string; kind: "sync" | "marketplace" }) {
  const [query, setQuery] = useState("");
  const Studio = kind === "sync" ? CapabilitySyncStudio : CapabilityMarketplace;
  return (
    <div className="flex min-h-[60vh] flex-col gap-4">
      <label className="flex items-center gap-2 rounded-md border border-content/10 px-2.5 py-1.5">
        <Search className="size-3.5 text-content/45" />
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder={kind === "sync" ? "Filter capabilities" : "Search GitHub"}
          className="min-w-0 flex-1 bg-transparent text-[13px] text-content outline-none placeholder:text-content/35"
        />
      </label>
      <div className="min-h-0 flex-1">
        <PageErrorBoundary>
          <Suspense fallback={<Loader className="size-4 animate-spin text-content/45" />}>
            <Studio path={cwd} query={query} />
          </Suspense>
        </PageErrorBoundary>
      </div>
    </div>
