...
const gapRaw = parseFloat(
  getComputedStyle(document.documentElement).getPropertyValue("--stream-gap")
);
const gap = Number.isFinite(gapRaw) ? gapRaw : 60;

// Bumped per call so a replay abandons the previous timer chain.
let run = 0;
...
function stream() {
  ...
  (function next(n) {
    if (token !== run || n >= spans.length) return;
