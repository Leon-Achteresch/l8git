...
```html
<button class="t-toggle" role="switch" aria-checked="false" data-on="false">
  <span class="t-toggle-thumb"></span>
</button>
```

Toggle `data-on` and keep `aria-checked` in sync with it — `role="switch"`
is only valid with `aria-checked`. Add `.is-init` on first interaction so the
