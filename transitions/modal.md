...
```html
<div class="t-modal" role="dialog" aria-modal="true" aria-label="…">…</div>
```

This recipe owns the open/close motion only. A dialog also needs an
accessible name, focus moved into it on open, focus kept inside while
open, Escape to dismiss, and focus returned to the trigger on close —
wire those yourself or use `<dialog>`/a headless dialog primitive.
...
// CSS time values may be authored in s or ms; normalise to ms.
...
function closeModal() {
  ...
  closeTimer = setTimeout(() => modal.classList.remove("is-closing"), closeMs);
}
```
