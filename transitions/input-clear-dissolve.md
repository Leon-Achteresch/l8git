...
<div class="t-clear has-value">
  ...
  <button type="button" class="t-clear-btn" aria-label="Clear">…</button>
</div>
```
...
/* White streaks would vanish under multiply — lighten instead. */
[data-theme="dark"] .t-clear-glow { mix-blend-mode: screen; }
...
document.querySelectorAll(".t-clear").forEach((wrap) => {
  ...
  function clearWithAnimation() {
    ...
    // Reduced motion: land on the cleared state directly, no tween.
    if (matchMedia("(prefers-reduced-motion: reduce)").matches) {
      input.value = "";
      mirror.textContent = "";
      wrap.classList.remove("has-value", "is-clearing");
      return;
    }
    clearing = true;
