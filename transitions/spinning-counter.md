...
**Incomplete — this recipe needs JS that it does not ship.** The CSS above
only owns the reel's resting geometry and the mask. Building the columns,
writing each strip's `transform`, staggering the columns and decaying the
`feGaussianBlur` all have to be driven from your own code; the stagger in
particular has to be computed as a number in JS, since `var(--reel-stagger)`
cannot be used inside JS arithmetic.
