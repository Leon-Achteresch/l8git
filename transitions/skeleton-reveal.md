...
/* Grid stacking rather than absolute children, so the wrap keeps the
   intrinsic height of the tallest layer instead of collapsing to 0. */
.t-skel { position: relative; display: grid; }
.t-skel-skeleton,
.t-skel-content {
  grid-area: 1 / 1;
}
...
function replay() {
  ...
  revealTimer = setTimeout(() => skel.classList.add("is-revealed"), total);
}
```
