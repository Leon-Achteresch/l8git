...
fn migrate(conn: &Connection) -> rusqlite::Result<()> {
    ...
    if current < 10 {
        crate::monocode::notes::ensure_notes_table(conn)?;
        conn.execute(
...
fn upsert_session(conn: &Connection, session: &SessionUpsert) -> rusqlite::Result<SessionSummary> {
    ...
    let git = crate::monocode::fs::git_info_for(&crate::monocode::fs::expand_home(&session.cwd));
    let branch = session
        .branch
        .as_deref()
        .map(str::trim)
...
fn list_by_project(conn: &Connection, cwd: &str) -> rusqlite::Result<Vec<SessionSummary>> {
    let git = crate::monocode::fs::git_info_for(&crate::monocode::fs::expand_home(cwd));
