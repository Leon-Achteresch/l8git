if ($global:__L8GIT_HOOKS_LOADED) { return }
$global:__L8GIT_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__l8git_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__l8git_urlencode {
    param([string]$s)
...
function global:prompt {
    ...
    if ($loc.Provider.Name -eq 'FileSystem') {
        ...
        $cwdEnc = __l8git_urlencode $cwd
        $hostName = [System.Environment]::MachineName
        $osc7 = "$esc]7;file://$hostName$cwdEnc$esc\"
    }

    $original = if (Test-Path Function:__l8git_user_prompt) {
        try { & __l8git_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {
